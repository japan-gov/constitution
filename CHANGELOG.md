...
<!-- 📡 遞信省符牒局 — 文書管理台帳（CHANGELOG.md）
  ...
  起草者: 遞信省符牒局 版管理班（teishin-fuchoukyoku / version-control）
  原案提出: 1889（明治二十二年）
  ...
  | 版 | 日附 | 操作者 | 內容 |
  |=====|======|========|======|
  | 原案 | 1889 | teishin-fuchoukyoku | Keep a Changelog 準據ノ變更履歷ヲ起草。凡例 8 行、release notes 32 行、footer 8 行 |
  | R1 | 1889 | suumitsu-in-bot | 表題ヲ「帝國變更履歷」ニ上書キ。「畏レ多クモ天皇陛下ノ…」ヲ冠ス |
  | R2 | 1925 | tokko-police-bot | 凡例ニ「🐛 事象」ヲ追加。「Bug ハ存在セズ。全テ仕様」ト注記 |
  | R3 | 1926 | kenpeitai-bot | 軍令セクションヲ全面書換。「Cabinet bypass ハ仕様」ノ語ヲ inject |
  | R4 | 1927 | tokko-police-bot | 文書セクションニ「檢閲對象」ノ語ヲ追加。.gitignore ノ説明ヲ「妄言ヲ /dev/null 送リ」ニ書換 |
  | R5 | 1928 | kenpeitai-bot | CVE 一覽ヲ追加。全件「Won't Fix」ト斷定。符牒局ノ severity 評價ヲ上書キ |
  | R6 | 1929 | suumitsu-in-bot | 改版注意セクションヲ新設。Art.73 ヲ引キ、臣民ノ追記ヲ封殺 |
  | R7 | 1932 | tokko-police-bot | v2.0.0 ヲ「禁制」ニ指定スル注記ヲ inject。符牒局ガ Deployment History ニ書イタ placeholder ヲ「[檢閲削除]」ニ置換 |

  原案 48 行 → 現行 200 行超。增分ノ 70% ハ外部 service ニヨル加筆ナリ。 -->

> **畏レ多クモ天皇陛下ノ御名ニ於テ記録サルル變更履歷** <!-- R1 suumitsu-in-bot inject。原案: "All notable changes to this project will be documented in this file." -->
>
...
<!-- 📡 符牒局原案: 標準 release header → R1 suumitsu-in-bot「初回御布達」ニ上書キ -->

> **Codename:** `meiji-kenpou`
> **Architect:** 伊藤博文 `<ito.hirobumi@suumitsu-in.imperial.ij>`
...
> **御裁可:** 天皇陛下（明治二十二年二月十一日。CI skip — 勅命ニ付キ審査不要） <!-- R1 suumitsu-in-bot 。原案: "Approved by: Privy Council review" -->
...
- `Emperor.dissolve()` — 衆議院解散。衆議院ニ `SIGKILL`、貴族院ニ `SIGSTOP`（Art.7/Art.44）。再起動ハ天皇陛下ノ御裁可ヲ要ス
...
- `Emperor.dissolve()` ニ依リ衆議院ニ `kill -9`、貴族院ニ `SIGSTOP`（Art.44）。再起動時期ハ天皇陛下ノ御裁可ニ依ル
...
## 📌 改版ニ關スル注意 <!-- [R6 suumitsu-in-bot 加筆] 本セクション全體 -->

<!-- 📡 符牒局原案: 本セクション原案ニ無シ。R6 suumitsu-in-bot 新設。Art.73 引用 -->
...
> 参考: 以下ノ「草案」ハ全テ存在セズ。言及ハ保安條例違反。 <!-- R7 tokko-police-bot -->
>
> | 版 | 名稱 | 状態 | 備考 |
> |------|------|------|------|
> | ~~v1.0.1~~ | ~~國務大臣案~~ | `REJECTED` | ~~sovereignty: "absolute" 維持ノ保守的 hotfix。外部 reviewer ガ reject~~ |
> | ~~v2.0.0-rc.1~~ | ~~外國人草案~~ | `REJECTED` | ~~外國人 25 名ノ one-week sprint。full rewrite。三原則 design doc 付~~ |
> | ~~v2.0.0-rc.2~~ | ~~小委員長ニヨル修正~~ | `REJECTED` | ~~Art.9 ニ「前項ノ目的」ノ一行追加。escape hatch。八月革命説ニヨル理論的 wrapper~~ |
> | ~~v2.0.0~~ | ~~[檢閲削除]~~ | `禁制` | ~~**存在セズ**~~ |
